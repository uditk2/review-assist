...
<p align="center">
  <img src="https://raw.githubusercontent.com/uditk2/review-assist/main/docs/mcp-distillation.png" alt="Six numbered steps. The coding agent spawns two role-locked subagents. The Author holds the session transcript and supplies what the diff cannot show, but has no submission tools. The Intent Reviewer never sees the transcript; it reads the diff cold, and its questions come from its baseline set, the diff, and the repository's own .reviewer/ house rules served by get_reviewer_instructions. Every question and answer passes through the server, which registers a different tool surface per role; get_role_definitions, import_session and manage_consent belong to neither and are called by the coding agent itself. record_interview_round hands the Reviewer's questions back with a q_id each, the Author answers by id, and the Reviewer reads the answers verbatim. submit_document gates the write to .intent/&lt;branch&gt;.json behind repository consent, interview attestation and five local checks. The Reviewer also reports back to the session what the document could not settle." width="820">
</p>
